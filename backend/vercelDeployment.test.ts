...
describe("Vercel deployment safety", () => {
  ...
  it("protects local credential files and routes API traffic through the Vercel function", () => {
    ...
    expect(readProjectFile("backend/_core/env.ts")).toContain("NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY");
    expect(readProjectFile("backend/_core/app.ts")).toContain("publishableKey: ENV.clerkPublishableKey");
  });
